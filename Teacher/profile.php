...
if (mysqli_connect_errno()) {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        if (isset($_POST['update_info'])) {
            ...
            // Use prepared statements to update the teacher's information
            $stmt = $con->prepare("UPDATE teacher SET name = ?, phone = ?, email = ?, gender = ?, dob = ?, picture = ? WHERE teacher_id = ?");
            $stmt->bind_param("ssssssi", $name, $phone, $email, $gender, $dob, $picture, $teacherId); // Update variable names here
            $result = $stmt->execute();
            ...
            // Retrieve the current teacher information
            $stmt = $con->prepare("SELECT * FROM teacher WHERE teacher_id = ?");
            ...
            if (mysqli_num_rows($result) > 0) {
                ...
                if (password_verify($password, $storedPassword)) {
                    ...
                    if ($newPassword === $confirmPassword) {
                        ...
                        // Use prepared statements to update the teacher's password
                        $stmt = $con->prepare("UPDATE teacher SET password = ? WHERE teacher_id = ?");
                        $stmt->bind_param("si", $hashedPassword, $teacherId); // Update variable names here
                        $result = $stmt->execute();

                        if ($result) {
                            $msg = "Teacher password updated successfully.";
                        } else {
                            $error = "Error updating teacher password.";
                        }
                    } else {
