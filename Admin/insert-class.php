<?php
include('connection.php');

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  // Taking all values from the class form data(input)
  $name = $_POST['name'];
  $form = $_POST['form'];
  $teacher_id = $_POST['teacher_id'];
  $day = $_POST["day"];
  $timeSlot = $_POST["time_slot"];

  // Perform necessary validations on the input data

  // Prepare the SQL statement with parameter placeholders
  $sql = "INSERT INTO class (name, form, teacher_id, day, time_slot) VALUES (?, ?, ?, ?, ?)";

  // Create a prepared statement
  $stmt = mysqli_prepare($con, $sql);

  // Bind the parameters with the corresponding values
  mysqli_stmt_bind_param($stmt, "sssss", $name, $form, $teacher_id, $day, $timeSlot);

  // Execute the prepared statement
  if (mysqli_stmt_execute($stmt)) {
    echo '<script language="javascript">
            alert("Class added successfully.");
            window.location.replace("class.php");
          </script>';
  } else {
    echo "ERROR: Failed to add class. " . mysqli_error($con);
  }

  // Close the prepared statement
  mysqli_stmt_close($stmt);
}

mysqli_close($con);
?>
